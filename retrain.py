...
Usage:
    ...
    python retrain.py --reference ../data-pack/data/baseline.csv --current ../data-pack/data/drifted.csv

    # Full stress test with holdout + post-deploy eval:
    python retrain.py \\
...
def append_audit(event: str, detail: dict) -> None:
    ...
    entry = {"timestamp": datetime.datetime.utcnow().isoformat(), "event": event, **detail}
    with open(AUDIT_LOG_PATH, "a") as f:
        f.write(json.dumps(entry) + "\n")


def train_model_on_df(df: pd.DataFrame, contamination: float = 0.03, n_estimators: int = 100):
    """Train IsolationForest on a DataFrame, return (model, scaler, anomaly_rate, n_rows)."""
...
def main():
    ...
    if args.auto_approve:
        ...
        print("=" * 60)
        answer = input("Drift detected. Model v2 registered as staging. Promote to production? [y/N] ").strip().lower()
        approved = answer == "y"
