...
def run_runbook(script_args: str, service: str, dry_run: bool, timeout_s: int = 30) -> bool:
    """Execute runbook script. Returns True on exit code 0."""
    import shlex
    script_parts = shlex.split(script_args)
    # Changed /bin/bash to bash for windows compatibility, assuming bash is in PATH
    cmd = ["bash"] + script_parts + ["--service", service]
    if dry_run:
        cmd.append("--dry-run")
    log.info("RUNBOOK_EXEC", script=script_args, service=service, dry_run=dry_run)
    try:
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=timeout_s)
        log.info("RUNBOOK_RESULT", script=script_args, service=service,
                 returncode=result.returncode,
                 stdout=result.stdout.strip(), stderr=result.stderr.strip())
        return result.returncode == 0
    except subprocess.TimeoutExpired:
        log.error("RUNBOOK_TIMEOUT", script=script_args, service=service, timeout_s=timeout_s)
        return False
    except Exception as exc:
        log.error("RUNBOOK_ERROR", script=script_args, service=service, error=str(exc))
        return False
...
def validate_runbook(runbook: str, cfg: dict, alertname: str, raw_decision: str) -> bool:
    ...
    # Also include the base scripts from multi_step_map
    if runbook in registry:
        return True
    
    # Check if runbook is just script with args, like 'runbooks/multi_step_deploy.sh --step-a'
    runbook_script = runbook.split()[0]
    if runbook_script in registry or runbook_script in ["runbooks/multi_step_deploy.sh"]:
        return True
...
def _process_alert_locked(
    ...
    if multi_steps:
        for step in multi_steps:
            if not run_runbook(step, service, dry_run=True, timeout_s=timeout_s):
                log.error("DRY_RUN_FAIL", runbook=step, service=service)
                return
        log.info("DRY_RUN_PASS", runbook="MultiStepDeploy", service=service)
    else:
